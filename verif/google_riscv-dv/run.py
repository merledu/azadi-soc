...
def parse_iss_yaml(iss, iss_yaml, isa, setting_dir, debug_cmd):
    ...
    for entry in yaml_data:
        if entry["iss"] == iss:
            ...
            cmd = re.sub(
                "\<path_var\>", get_env_var(entry["path_var"], debug_cmd=debug_cmd), cmd
            )
...
def run_csr_test(
    ...
    cmd = (
        ...
        + (" --xlen {}".format(re.search(r"(?P<xlen>[0-9]+)", isa).group("xlen")))
    ...
    for test in test_list:
        ...
        if iterations > 0:
            ...
            if test["test"] == "riscv_csr_test":
                ...
                logging.info(
                    "Running {} with {} batches".format(test["test"], batch_cnt)
                )
...
def gen(test_list, argv, output_dir, cwd):
    ...
    if argv.simulator == "ius":
        ...
        logging.debug("Disable return_code checking for {}".format(argv.simulator))
...
def run_assembly(
    ...
    cmd = "{} -static -mcmodel=medany \
         ...
         -I{}/user_extension \
         -T{}/scripts/link.ld {} -o {} ".format(
        get_env_var("RISCV_GCC", debug_cmd=debug_cmd), asm_test, cwd, cwd, gcc_opts, elf
    )
...
def run_assembly_from_dir(
    asm_test_dir, iss_yaml, isa, mabi, gcc_opts, iss, output_dir, setting_dir, debug_cmd
):
    ...
    if result:
        ...
        logging.info(
            "Found {} assembly tests under {}".format(len(asm_list), asm_test_dir)
        )
        ...
        logging.error("No assembly test(*.S) found under {}".format(asm_test_dir))


def run_c(
    c_test, iss_yaml, isa, mabi, gcc_opts, iss_opts, output_dir, setting_dir, debug_cmd
):
    ...
    cmd = "{} -mcmodel=medany -nostdlib \
         ...
         -I{}/user_extension \
         -T{}/scripts/link.ld {} -o {} ".format(
        get_env_var("RISCV_GCC", debug_cmd=debug_cmd), c_test, cwd, cwd, gcc_opts, elf
    )
    cmd += " -march={}".format(isa)
    cmd += " -mabi={}".format(mabi)
    run_cmd_output(cmd.split(), debug_cmd=debug_cmd)
...
def run_c_from_dir(
    c_test_dir, iss_yaml, isa, mabi, gcc_opts, iss, output_dir, setting_dir, debug_cmd
):
...
def iss_cmp(test_list, iss, output_dir, stop_on_first_error, exp, debug_cmd):
    ...
    for test in test_list:
        for i in range(0, test["iterations"]):
            ...
            logging.info(
                "Comparing ISS sim result {}/{} : {}".format(
...
def parse_args(cwd):
    ...
    parser.add_argument(
        "--iss_timeout", type=int, default=10, help="ISS sim timeout limit in seconds"
    )
    ...
    parser.add_argument(
        "--asm_test", type=str, default="", help="Directed assembly tests"
    )
    parser.add_argument("--c_test", type=str, default="", help="Directed c tests")
    parser.add_argument(
        ...
        if args.steps == "all" or re.match(".*gen.*", args.steps):
            ...
            if len(asm_directed_list) != 0:
                for test_entry in asm_directed_list:
                    ...
                    path_asm_test = os.path.expanduser(test_entry.get("asm_test"))
                    if path_asm_test:
                        # path_asm_test is a directory
                        ...
                        # path_asm_test is an assembly file
                            ...
                            if not args.debug:
                                logging.error("{} does not exist".format(path_asm_test))
                                sys.exit(RET_FAIL)
