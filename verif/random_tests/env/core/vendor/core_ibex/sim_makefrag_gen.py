...
def run_ibex_config(output_type, extra_args=None):
    ...
    result = subprocess.run(
        ibex_config_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def gen_vcs_makefrag():
    vcs_compile_opts = run_ibex_config(
        "vcs_opts",
...
def gen_riviera_makefrag():
    riviera_compile_opts = run_ibex_config(
        "riviera_compile_opts",
        ["--ins_hier_path", "core_ibex_tb_top", "--string_define_prefix", "IBEX_CFG_"],
    )

    riviera_sim_opts = run_ibex_config(
        "riviera_sim_opts",
        ["--ins_hier_path", "core_ibex_tb_top", "--string_define_prefix", "IBEX_CFG_"],
    )

    return ("COMPILE_OPTS += {0}" "SIM_OPTS += {1}").format(
        riviera_compile_opts, riviera_sim_opts
    )


def gen_questa_makefrag():
    questa_compile_opts = run_ibex_config(
        "questa_compile_opts",
        ["--ins_hier_path", "core_ibex_tb_top", "--string_define_prefix", "IBEX_CFG_"],
    )

    questa_sim_opts = run_ibex_config(
        "questa_sim_opts",
        ["--ins_hier_path", "core_ibex_tb_top", "--string_define_prefix", "IBEX_CFG_"],
    )

    return ("COMPILE_OPTS += {0}" "SIM_OPTS += {1}").format(
        questa_compile_opts, questa_sim_opts
    )


def gen_xlm_makefrag():
    xlm_compile_opts = run_ibex_config(
        "xlm_opts",
        ["--ins_hier_path", "core_ibex_tb_top", "--string_define_prefix", "IBEX_CFG_"],
    )
...
def gen_dsim_makefrag():
    dsim_compile_opts = run_ibex_config(
        "dsim_compile_opts",
        ["--ins_hier_path", "core_ibex_tb_top", "--string_define_prefix", "IBEX_CFG_"],
    )
...
def main():
    ...
    argparser.add_argument("sim", help="Name of the simulator", choices=sim_fns.keys())

    argparser.add_argument(
        "config", help="Ibex config to generate makefile fragment for"
    )

    argparser.add_argument("ibex_top", help="Path to the top of an ibex repository")

    argparser.add_argument(
        "makefrag_name", help="Filename of the makefile fragment to write"
    )
    ...
    ibex_config_path = os.path.join(
        os.getcwd(), "util/ibex_config.py"
    )  # os.path.join(args.ibex_top, 'util/ibex_config.py')
    ibex_config_name = args.config
    ibex_config_filename = os.path.join(
        os.getcwd(), "ibex_configs.yaml"
    )  # "/home/asabir/projects/azadi_tsmc/azadi-tsmc/rtl/rtl-v1.3/ibex_configs.yaml"
