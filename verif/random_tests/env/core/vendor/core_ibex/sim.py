...
_IBEX_ROOT = os.path.normpath(
    os.path.join(_CORE_IBEX, "../../../../../azadi-new/hw/vendor/lowrisc_ibex")
)
_RISCV_DV_ROOT = os.path.normpath(
    os.path.join(_IBEX_ROOT, "../../../../google_riscv-dv")
)
...
try:
    sys.path = [
        os.path.join(_CORE_IBEX, "riscv_dv_extension"),
...
def subst_opt(string, name, enable, replacement):
    ...
    if replacement is None:
        if needle in string:
            raise RuntimeError(
                "No replacement defined for {} "
                "(used in string: {!r}).".format(needle, string)
            )
        return string
...
def subst_env_vars(string, env_vars):
    ...
    for env_var in env_vars.split(","):
        ...
        if needle in string:
            ...
            if value is None:
                raise RuntimeError(
                    "Cannot substitute {} in command because "
                    "the environment variable ${} is not set.".format(needle, env_var)
                )
...
def get_simulator_cmd(simulator, yaml_path, enables):
    ...
    return (
        [
            subst_cmd(arg, enables, entry["compile"], env_vars)
...
def rtl_compile(compile_cmds, output_dir, lsf_cmd, opts):
    ...
    for cmd in compile_cmds:
        cmd = subst_vars(cmd, {"out": output_dir, "cmp_opts": opts})
...
def get_test_sim_cmd(base_cmd, test, idx, output_dir, bin_dir, lsf_cmd):
    ...
    """
    sim_cmd = (
        base_cmd + " " + test["sim_opts"].replace("\n", " ")
        if "sim_opts" in test
        else base_cmd
    )
    ...
    if not os.path.exists(binary):
        raise RuntimeError(
            ...
            "expected binary at {!r}.".format(idx, test_name, binary)
        )
...
def rtl_sim(sim_cmd, test_list, seed_gen, opts, output_dir, bin_dir, lsf_cmd):
    ...
    for test in test_list:
        for i in range(test["iterations"]):
            ...
            cmd_list.append(
                get_test_sim_cmd(it_cmd, test, i, output_dir, bin_dir, lsf_cmd)
            )
...
def compare_test_run(test, idx, iss, output_dir, report):
    ...
    """
    test_name = test["test"]
    elf = os.path.join(output_dir, "instr_gen/asm_test/{}.{}.o".format(test_name, idx))
    ...
    rtl_dir = os.path.join(output_dir, "rtl_sim", "{}.{}".format(test_name, idx))
    ...
    compare_result = compare_trace_csv(
        rtl_csv, iss_csv, "ibex", iss, report, **test.get("compare_opts", {})
    )
...
def gen_cov(base_dir, simulator, lsf_cmd):
    ...
    for entry in os.scandir(base_dir):
        ...
        if "seed" in entry.name:
            logging.info(
                "Searching %s/%s for coverage database" % (base_dir, entry.name)
            )
    ...
    if simulator == "vcs":
        cov_cmd = (
            "urg -full64 -format both -dbname test.vdb "
...
def read_seed(arg):
    ...
    try:
        ...
        raise argparse.ArgumentTypeError(
            "Bad argument for --seed ({}): "
            "must be a non-negative integer.".format(arg)
        )
...
def main():
    ...
    parser.add_argument("--o", type=str, default="out", help="Output directory name")
    parser.add_argument(
        "--testlist",
        help="Regression testlist",
        default=os.path.join(_CORE_IBEX, "riscv_dv_extension", "testlist.yaml"),
    )
    parser.add_argument(
        "--test",
        type=str,
    ...
    parser.add_argument(
        ...
        default=os.path.join(_CORE_IBEX, "yaml", "rtl_simulation.yaml"),
    )
    parser.add_argument(
        "--iss",
        default="spike",
    ...
    parser.add_argument("--en_cov", action="store_true", help="Enable coverage dump")
    parser.add_argument("--en_wave", action="store_true", help="Enable waveform dump")
    parser.add_argument(
        "--steps", type=str, default="all", help="Run steps: compile,sim,compare,cov"
    )
    parser.add_argument(
        "--lsf_cmd",
        type=str,
        help=("LSF command. Run locally if lsf " "command is not specified"),
    )
    ...
    sg.add_argument(
        ...
        metavar="S",
        help=(
            "Randomization seed for the only iteration of each "
    ...
    sg.add_argument(
        ...
        metavar="S",
        help=(
            "Randomization seed for the first iteration of each "
            "test. Following iterations will use seeds S+1, "
            "S+2, and so on."
        ),
    )
    sg.add_argument(
        "--iterations", type=int, help="Override the iteration count in the test list"
    )
    ...
    if steps["sim"] or steps["compare"]:
        ...
        if seed_gen.fixed:
            ...
            if args.iterations != 1:
                logging.error("Cannot set multiple --iterations with a " "fixed seed.")
                return RET_FAIL

        process_regression_list(
            args.testlist, args.test, args.iterations or 0, matched_list, _RISCV_DV_ROOT
        )
        if not matched_list:
            raise RuntimeError("Cannot find %s in %s" % (args.test, args.testlist))
