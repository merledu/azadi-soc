...
_IBEX_ROOT = os.path.normpath(
    os.path.join(
        os.path.dirname(__file__), "../../../../../../azadi-new/hw/vendor/lowrisc_ibex"
    )
)
...
def process_ibex_sim_log(ibex_log, csv, full_trace=1):
    ...
    try:
        with open(ibex_log, "r") as log_fd, open(csv, "w") as csv_fd:
            count = _process_ibex_sim_log_fd(
                log_fd, csv_fd, True if full_trace else False
            )
...
def expand_trace_entry(trace, operands):
    ...
    trace.instr, operands = convert_pseudo_instr(trace.instr, operands, trace.binary)
...
def main():
    ...
    parser.add_argument(
        "--full_trace", type=int, default=1, help="Enable full log trace"
    )

    args = parser.parse_args()

    _process_ibex_sim_log_fd(args.log, args.csv, True if args.full_trace else False)
